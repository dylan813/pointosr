...
class HumanDataset(Dataset):
    ...
    def __init__(self, data_dir, split,
        ...
        # Get all class directories for searching
        class_dirs = list(self.dir_to_class_idx.keys())
        
        with open(split_filename, 'r') as f:
            for line in f:
                # Each line contains a filename without directory prefix
                filename = line.strip()
                if not filename: continue # Skip empty lines
                
                # Check each class directory to find the file
                file_found = False
                for class_dir in class_dirs:
                    # Construct path with class directory
                    filepath = os.path.join(data_dir, class_dir, filename)
                    if os.path.isfile(filepath):
                        # File found, add to list with proper class prefix
                ...
                if not file_found:
                    logging.warning(f"File {filename} not found in any class directory.")
    ...
    def __getitem__(self, idx):
        ...
        if current_num_points > self.num_points:
            # Sample points if too many - use random sampling for all splits
            # This avoids CUDA-related issues with FPS
